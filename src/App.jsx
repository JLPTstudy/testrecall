...
const SHARED_RULES = `
【词汇提取范围（重要）】
提取文中出现的【所有值得记忆的词汇】，不要主观判断"够不够N1"而跳过。只排除以下明显基础词：
- 极简单的动词（する、ある、いる、来る、行く、見る、食べる、言う等）
- 助词、助动词、接续词本身（が、は、で、に、て、から、ので等）
- 数字、时间、极简单的名词（人、日、年、月等）
其余出现在题目或选项中的名词、动词、形容词、副词，一律提取，包括看起来"普通"的汉字词。

【类型判断规则（严格执行）】
- vocabulary: 单个词语（名词/动词/形容词/副词/汉字词）。能单独查到词典的独立词汇。
- grammar: N1语法句型，含助词组合或接续形式（如：〜にもかかわらず、〜を皮切りに、〜に際して、〜ずにはおかない、〜に相違ない）
- collocation: 两个以上词组成的惯用表达/惯用句，整体才有特殊意义（如：気が置けない、手が込む、一石二鳥、腹を割る）。拆开后含义消失才算collocation。

【提取要求】
1. 提取题目正文和全部选项（选择题4个选项都要检查）中的所有词汇，不要因为觉得"太简单"而跳过
2. 所有N1语法句型必须提取（type=grammar），哪怕只出现一次，绝对不能遗漏
3. 语法的term字段写完整句型（如：〜にもかかわらず），connection字段写接续方式（如：名词/普通形+にもかかわらず）
4. 词汇若在文中以变形出现（如动词活用、形容词变形），term字段必须写辞书形/原形（如：製作された→製作する、見かねた→見かねる、悔しくて→悔しい）
5. 词汇的reading字段填写辞书形的假名读音
6. collocation和vocabulary不要搞混：单独词→vocabulary，多词惯用→collocation
7. 不要提取「文法」「語彙」「読解」「聴解」「問題」等章节标题
8. 提取完毕后，回顾一遍是否有遗漏的词汇或语法点，再输出结果`
